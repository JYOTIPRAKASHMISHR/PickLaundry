...
public class Seconddryclean extends AppCompatActivity {

    private EditText shirtInput, pantsInput, othersInput;
    private TextView totalPiecesText, totalPriceText, othersMessage;
    private Button continueButton;

    private final int PRICE_PER_SHIRT = 20; // Dry clean prices
    private final int PRICE_PER_PANTS = 25;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_seconddryclean);

        // ✅ Initialize Firebase references
        mAuth = FirebaseAuth.getInstance();
        orderRef = FirebaseDatabase.getInstance().getReference("DryCleanOrders");
                ...
                if (snapshot.exists()) {
                    ...
                    if (orderId != null) {
                        orderRef.child(orderId).setValue(orderData)
                                ...
                                .addOnFailureListener(e -> Toast.makeText(Seconddryclean.this, "Order save failed: " + e.getMessage(), Toast.LENGTH_SHORT).show());
                    } else {
                        Toast.makeText(Seconddryclean.this, "Failed to generate order ID.", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(Seconddryclean.this, "User details not found.", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(Seconddryclean.this, "Database error: " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void saveOrderStatus(String orderId) {
        ...
        statusRef.child(orderId).setValue(statusData)
                .addOnSuccessListener(unused -> {
                    Toast.makeText(Seconddryclean.this, "Order & status saved successfully!", Toast.LENGTH_SHORT).show();
                    clearInputs();
                    navigateToNextPage();
                })
                .addOnFailureListener(e -> Toast.makeText(Seconddryclean.this, "Status save failed: " + e.getMessage(), Toast.LENGTH_SHORT).show());
    }

    private void navigateToNextPage() {
        Intent intent = new Intent(Seconddryclean.this, Thirddryclean.class);
