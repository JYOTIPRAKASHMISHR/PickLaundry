...
public class Secondwashandfolds extends AppCompatActivity {

    private EditText shirtInput, pantsInput, othersInput;
    private TextView totalPiecesText, totalPriceText, othersMessage;
    private Button continueButton;

    private final int PRICE_PER_SHIRT = 8; // 🧺 Wash & Fold prices
    private final int PRICE_PER_PANTS = 12;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_secondwashandfolds);

        // ✅ Initialize Firebase references
        mAuth = FirebaseAuth.getInstance();
        orderRef = FirebaseDatabase.getInstance().getReference("Washandfolds");
                ...
                if (snapshot.exists()) {
                    ...
                    if (orderId != null) {
                        orderRef.child(orderId).setValue(orderData)
                                ...
                                .addOnFailureListener(e -> Toast.makeText(Secondwashandfolds.this, "Order save failed: " + e.getMessage(), Toast.LENGTH_SHORT).show());
                    } else {
                        Toast.makeText(Secondwashandfolds.this, "Failed to generate order ID.", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(Secondwashandfolds.this, "User details not found.", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(Secondwashandfolds.this, "Database error: " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void saveOrderStatus(String orderId) {
        ...
        statusRef.child(orderId).setValue(statusData)
                .addOnSuccessListener(unused -> {
                    Toast.makeText(Secondwashandfolds.this, "Order & status saved successfully!", Toast.LENGTH_SHORT).show();
                    clearInputs();
                    navigateToThirdPage();
                })
                .addOnFailureListener(e -> Toast.makeText(Secondwashandfolds.this, "Status save failed: " + e.getMessage(), Toast.LENGTH_SHORT).show());
    }

    private void navigateToThirdPage() {
        Intent intent = new Intent(Secondwashandfolds.this, Thirdwashandfolds.class);
